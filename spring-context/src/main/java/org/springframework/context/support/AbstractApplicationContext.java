...
/**
 * context. context. context的抽象实现。ApplicationContext}接口
 * 。不强制配置使用的存储类型;简单地实现公共上下文功能。使用模板方法设计模式，需要具体的子类来实现抽象方法。
 * <p>
...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			//该方法用于容器的准备，包括设置上下文状态，获取属性，验证必要的属性 记录启动时间等。
			prepareRefresh();
			// Tell the subclass to refresh the internal bean factory.
			//该方法 刷新（存在摧毁） BeanFactory 并设置id 和获取 getBeanFactory。
			...
			try {
				...
				//所有的 beanDefinition 已经加载，但是还没有实例化允许在子类中对 beanFactory 进行扩展处理。
	...
	/**
	 * 准备这个上下文以便刷新，设置它的启动日期和活动标志，以及执行属性源的任何初始化。
	 */
...
//		存储预刷新ApplicationListeners…
	...
	/**
	 * 完成此上下文bean工厂的初始化，初始化所有剩余的单例bean。依赖注入
	 */
